...
def split_data_by_catalyst(df, n_train, n_test, seed, feature_cols, target_col):
    """
    Split data by catalyst into train/test sets (no validation set).

    Args:
        df: DataFrame with catalyst data
        n_train: Number of training catalysts
        n_test: Number of test catalysts
        seed: Random seed
        feature_cols: List of feature column names
        target_col: Target column name

    Returns:
        X_train, y_train, X_test, y_test, train_cats, test_cats
    """
    np.random.seed(seed)

    all_catalysts = df[CATALYST_ID].unique()
    n_total_needed = n_train + n_test

    if len(all_catalysts) < n_total_needed:
        raise ValueError(
            f"Not enough catalysts: need {n_total_needed}, have {len(all_catalysts)}"
        )

    # Sample catalysts
    test_cats = np.random.choice(all_catalysts, size=n_test, replace=False)
    remaining = [c for c in all_catalysts if c not in test_cats]

    train_cats = np.random.choice(remaining, size=n_train, replace=False)

    # Create splits
    train_mask = df[CATALYST_ID].isin(train_cats)
    test_mask = df[CATALYST_ID].isin(test_cats)
    ...
    return X_train, y_train, X_test, y_test, train_cats, test_cats
...
Test Set Metrics (n={len(results_list)} seeds):
  ...
  RMSE:
    Mean:  {np.mean(test_rmse):.4f}
    Std:   {np.std(test_rmse):.4f}
    Min:   {np.min(test_rmse):.4f}
    Max:   {np.max(test_rmse):.4f}
...
def main(
    ...
    print(
        f"  Test RMSE: {results_df['test_rmse'].mean():.4f} ± {results_df['test_rmse'].std():.4f}"
    )
