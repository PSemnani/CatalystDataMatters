...
def train_xgboost(
    ...
    if cross_val_masks is not None:
        ...
        # use fixed model instead of cross-validation
        if model_type == "default":
            model = default_xgb(seed=model_seed)
        elif model_type == "old_script":
            model = old_script_xgb(seed=model_seed)
            ...
            for j, feature_set in enumerate(feature_sets):
                ...
                for k, augm in enumerate(augmentations):
                    ...
                    results_rows.append(
                        {
                            "model_type": "xgboost"
                            + (f"_{model_type}" if model_type != "default" else ""),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="Model type: 'default', 'old_script', or 'optimized' (default: 'default')",
    )
    parser.add_argument(
        "--cross_validation",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="random_30",
        help="Parameter set for cross-validation: 'best_cases', 'avg_best', or 'random_x' where x is the number of random samples from all combinations (default: 'random_30')",
    )
    parser.add_argument(
        "--conditions",
        type=str,
